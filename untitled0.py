...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb 29 20:18:44 2020
...
def UCS(graph,source,dest):
    ...
    while True:
        ...
        c_weight = weight.pop(0)
        for neighbor in graph.neighbors(current):
            data = graph.get_edge_data(current,neighbor)
            cost+=data['weight']
            if(neighbor in queue):
                index = queue.index(neighbor)
                if(weight[index]>data['weight']+c_weight):
                    weight[index]=data['weight']+c_weight
                    index1 = child.index(neighbor)
                    parent[index1]=current
            elif neighbor not in visited:
                queue.append(neighbor)
                weight.append(data['weight']+c_weight)
                parent.append(current)
                child.append(neighbor)
        #take least weight child
...
#fddddddddffffffffffffffffffffffffffffffffffffffffffff
    cost=0
    flag=1
    queue = []
    visited = []
